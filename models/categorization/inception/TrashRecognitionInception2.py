...
def create_inception_model ():
    ...
    inception1 = inception_module(max_pool2, 32)

    max_pool3 = MaxPool2D((3, 3), strides=2, padding='same', name='max_pool3') (inception1)

    inception2 = inception_module(max_pool3, 64)
    inception3 = inception_module(inception2, 128)

    max_pool4 = MaxPool2D((3, 3), strides=2, padding='same', name='max_pool4') (inception3)

    inception4 = inception_module(max_pool4, 128)
    inception5 = inception_module(inception4, 256)
...
# train the model
history = inception_model.fit_generator(train_dataset, steps_per_epoch=2527, epochs=1000, verbose=2)

# save the model
json_model = inception_model.to_json()
with open("D:\\Workspaces\\python\\trash-recognition\\experiment-models\\trained\\inception-model.json", "w+") as json_file:
    json_file.write(json_model)
inception_model.save_weights(os.path.join(os.getcwd(), 'D:\\Workspaces\\python\\trash-recognition\\experiment-models\\trained\\inception-model.h5'))
